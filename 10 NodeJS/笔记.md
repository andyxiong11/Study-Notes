...
```js
npm init //文件夹npm初始化
npm install 包名 –g  （uninstall,update） //全局安装
npm install 包名 --save-dev (uninstall,update) //当前文件夹安装，dev指安装开发环境。等同于npm install 包名 -D
npm install 包名 -S //本地安装生产环境的包，等同于--save
npm install --production //只安装package.json中生产环境的包
npm list -g (不加-g，列举当前目录下的安装包)
npm view 包名 versions//查看可安装包版本
npm install 包名@1.1// 安装包的1.1.xx.xx的最新版本
